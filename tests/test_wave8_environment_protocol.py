...
def test_replay_frame_requires_measured_result_before_learning_claims() -> None:
    ...
    assert frame_without_result.status is EnvironmentTransitionStatus.NEEDS_MEASURED_RESULT
    assert (
    ...
    assert frame_with_measured_result.fingerprint() == frame_with_measured_result.fingerprint()
