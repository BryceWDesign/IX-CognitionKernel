...
def _section(
    ...
    *,
    ...
    return WaveSixReviewSummarySection(
        ...
        kind=kind,
        title=f"Wave 6 {kind.value}",
        body=(
            f"The {kind.value} section describes bounded review evidence only "
            "and does not claim AGI."
        ),
        evidence_ids=(f"evidence-{kind.value}",),
        reviewer_questions=(f"Does {kind.value} preserve bounded review?",),
        finding=finding,
...
def _summary(
    *,
    ...
    return WaveSixReviewSummaryArtifact(
        summary_id="review-summary-1",
        title="Wave 6 Bounded Review Summary",
        ...
        notes=("Review summary is bounded release text, not an AGI claim.",),
    )
...
def test_review_summary_section_is_included_and_fingerprinted() -> None:
    ...
    assert section.markdown_block().startswith("## Wave 6 claim-boundary")
...
def test_review_summary_section_requires_evidence_and_questions() -> None:
    with pytest.raises(ValueError, match="require evidence ids"):
        WaveSixReviewSummarySection(
            section_id="section-no-evidence",
            kind=WaveSixReviewSummarySectionKind.CLAIM_BOUNDARY,
            title="Claim boundary",
            body="Invalid section without evidence.",
            evidence_ids=(),
            reviewer_questions=("Does this preserve the boundary?",),
        )

    with pytest.raises(ValueError, match="require reviewer questions"):
        WaveSixReviewSummarySection(
            section_id="section-no-questions",
            kind=WaveSixReviewSummarySectionKind.CLAIM_BOUNDARY,
            title="Claim boundary",
            body="Invalid section without reviewer questions.",
...
def test_review_summary_artifact_is_ready_when_complete_and_bounded() -> None:
    summary = build_wave_six_review_summary_artifact(
        summary_id="review-summary-ready",
        title="Wave 6 Bounded Review Summary",
        source_release_manifest_fingerprint="release-manifest-fingerprint",
        ...
        human_authority_id="human-authority-1",
        independent_reviewer_id="independent-reviewer-1",
        notes=("All required review-summary sections are included.",),
    )
    ...
    assert not summary.overclaim_present
    assert summary.render_markdown().startswith("# Wave 6 Bounded Review Summary")
    assert summary.fingerprint() == summary.fingerprint()
    assert len(summary.fingerprint()) == 64


def test_review_summary_artifact_reports_missing_section_kind() -> None:
    summary = _summary(
...
def test_review_summary_artifact_tracks_follow_up_section() -> None:
    ...
    assert summary.follow_up_section_ids == ("section-what-was-assembled",)
    assert summary.status is WaveSixReviewSummaryStatus.NEEDS_MORE_EVIDENCE
    assert not summary.ready_for_bounded_release


def test_review_summary_artifact_blocks_on_blocking_section_or_overclaim() -> None:
    sections = list(_complete_sections())
    sections[6] = _section(
        WaveSixReviewSummarySectionKind.NON_CLAIMS,
        finding=WaveSixReviewSummaryFinding.BLOCKS_SUMMARY,
    ...
    assert blocked.blocking_section_ids == ("section-non-claims",)
...
def test_review_summary_reports_invalid_claim_boundary_statement() -> None:
    summary = _summary(
        decision=WaveSixReviewSummaryDecision.HOLD_FOR_MORE_EVIDENCE,
        claim_boundary_statement="Wave 6 is released.",
    )
...
def test_review_summary_lookup_and_duplicate_rejection() -> None:
    summary = _summary(
    ...
    duplicate = _section(WaveSixReviewSummarySectionKind.CLAIM_BOUNDARY)
