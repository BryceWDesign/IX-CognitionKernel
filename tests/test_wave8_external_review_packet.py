...
def test_external_review_packet_ready_with_replay_artifacts_roles_and_questions() -> None:
...
def test_external_review_packet_requires_ready_replay_report() -> None:
    ...
    assert any(
        finding.startswith("replay-report-not-ready")
...
def test_external_review_packet_requires_all_reviewer_roles() -> None:
    ...
    assert any(
        finding.startswith("missing-reviewer-roles")
        for finding in packet.findings
    )
